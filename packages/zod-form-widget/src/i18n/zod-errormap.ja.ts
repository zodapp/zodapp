...
const jaErrorMap: z.core.$ZodErrorMap = (issue) => {
  ...
  switch (issue.code) {
    ...
    case 'unrecognized_keys':
      message = `オブジェクトのキー '${issue.keys.join(', ')}' が識別できません。`;
      break;
    case 'invalid_union':
      message = '';
      break;
    ...
    case 'too_small':
      if (issue.origin === 'array') {
        if (issue.inclusive) message = `${issue.minimum}個以上の要素を設定する必要があります。`;
        else message = `${issue.minimum}個より多くの要素を設定する必要があります。`;
      } else if (issue.origin === 'string') {
        if (issue.inclusive) message = `${issue.minimum}文字以上で入力してください。`;
        else message = `${issue.minimum}文字より多く入力してください。`;
      } else if (issue.origin === 'number') {
        if (issue.inclusive) message = `${issue.minimum}以上の数値を入力してください。`;
    ...
    case 'too_big':
      if (issue.origin === 'array') {
        if (issue.inclusive) message = `${issue.maximum}個以下の要素を設定する必要があります。`;
        else message = `${issue.maximum}個より少ない要素を設定する必要があります。`;
      } else if (issue.origin === 'string') {
        if (issue.inclusive) message = `${issue.maximum}文字以下で入力してください。`;
        else message = `${issue.maximum}文字より少なく入力してください。`;
      } else if (issue.origin === 'number' || issue.origin === 'bigint') {
        if (issue.inclusive) message = `${issue.maximum}以下の数値を入力してください。`;
