...
- **pathFieldKeys**: path に含まれるキー。path から復元できるため通常はドキュメントには保存しませんが、collectionGroup クエリ等で直接フィールドとして参照したい場合に指定します。
 ...
 fieldKeys に含まれるキーは `storeSchema` では必須フィールドとして残ります。`updateSchema` では、取得済みの `dataSchema` をそのまま編集・再送しやすくする互換性のため optional のまま許容します。実際の create / update 書き込み時には、`beforeGenerate` / `beforeWrite` 実行時に identityParams から自動注入されます。
...
| **`updateSchema`** | `schema` + identity/document keys（optional） + createExcluded（as-is） | 更新用フォーム向け（互換性のため identityKey / fieldKeys を optional 許容） |
...
//   updatedAt?: Date;
// }
// ※ updateSchema で identityKey / fieldKeys が optional なのは、
//   取得済み dataSchema をそのまま編集・再送しやすくする互換性のため。
//   実際の update 書き込みでは beforeWrite が fieldKeys を identityParams から注入する
