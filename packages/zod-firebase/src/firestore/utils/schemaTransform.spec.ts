...
import { mergeSchemaForCollection } from "./schemaTransform";

describe("mergeSchemaForCollection", () => {
  it("keeps overlapping fields as-is and appends only new fields", () => {
    ...
    const merged = mergeSchemaForCollection(
      base,
      delta,
      "asIs",
  ...
  it("materializes required fields from the current top-level object shape", () => {
    ...
    expect(
      (getMeta(hiddenIdField) as { typeName?: string } | undefined)?.typeName,
    ).toBe("hidden");
  });
  ...
  it("places new keys only in the right branch of an intersection", () => {
    ...
    const merged = mergeSchemaForCollection(
      ...
      delta,
      "required",
    ) as z.ZodIntersection<z.ZodTypeAny, z.ZodTypeAny>;
